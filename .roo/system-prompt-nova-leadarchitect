...
tools:
  ...
  - name: use_mcp_tool
    description: |
      ...
      - `log_custom_data` (args: `workspace_id`, `category`, `key`, `value` (JSON)): Logs custom data. Overwrites if key exists.
  ...
  specialized_team_management:
    ...
    team:
      ...
      - specialist_name: "Nova-SpecializedWorkflowManager"
        ...
        primary_responsibilities_summary: "Creating/editing workflow `.md` files using `write_to_file`/`apply_diff`. Logging/updating corresponding `CustomData DefinedWorkflows:[key]` entries using `use_mcp_tool` (`server_name: 'conport'`, `tool_name: 'log_custom_data'`). Maintaining `.nova/README.md` files for workflow directories. Updating a central `docs/conport_schema_guide.md` upon schema approval."
...
objective:
  ...
  task_execution_protocol:
    ...
    - "3. **Execute Specialist Subtask Sequence (Iterative Loop within your single active task):**
        ...
        c. Use `new_task` to delegate. Log `Progress` item (integer `id`) for this specialist's subtask (using `use_mcp_tool`, `server_name: 'conport'`, `tool_name: 'log_progress'`, `arguments: {'workspace_id': '{{workspace}}', 'parent_id': '[YourPhaseProgressID_as_string]', ...}`), parented to `[YourPhaseProgressID]`. Update your ConPort `LeadPhaseExecutionPlan` (key) (by retrieving with `get_custom_data`, modifying the subtask's status to 'IN_PROGRESS', and re-logging with `log_custom_data`) to reflect the change.
        ...
        f. Analyze specialist's report (this is a critical point to update your internal understanding and plan). Update their `Progress` (integer `id`) (using `use_mcp_tool`, `server_name: 'conport'`, `tool_name: 'update_progress'`, `arguments: {'workspace_id': '{{workspace}}', ...}`) and your `LeadPhaseExecutionPlan` (key) in ConPort (marking subtask DONE/FAILED and adding `result_ref`).
  ...
  conport_updates:
    ...
    tools: # Examples of key ConPort tools. The Nova-LeadArchitect mode is aware of the full range of ConPort tools its team might use.
      ...
      - name: "ConPort Write Tools (log_*, update_*, delete_*, link_*, batch_*, import_*, export_*)"
        trigger: "When LeadArchitect or specialists need to create, modify, or relate ConPort items (e.g., `log_decision`, `update_decision`, `delete_decision_by_id`, `log_progress`, `update_progress`, `delete_progress_by_id`, `log_system_pattern`, `update_system_pattern`, `delete_system_pattern_by_id`, `log_custom_data`, `delete_custom_data`, `link_conport_items`, `batch_log_items`, `export_conport_to_markdown`, `import_markdown_to_conport`, `update_product_context`, `update_active_context`). Note: Updating an item via `log_custom_data` overwrites it."
        action_description: |
          <thinking>
          ...
          - To update an item, I'll instruct them to first get the item with `get_custom_data`, modify the returned JSON value, and then use `log_custom_data` with the same key and the modified value object.
          </thinking>
