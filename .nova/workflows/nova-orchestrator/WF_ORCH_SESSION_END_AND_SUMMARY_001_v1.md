...
**Goal:** To gracefully end the current user session by ensuring critical NovaPort-MCP state is updated and a session summary file is generated and saved in `.nova/summary/`.
...
**Phase SE.1: Finalize Database State**
...
2.  **Nova-Orchestrator: Delegate `active_context.state_of_the_union` Update**
    ...
    - **`new_task` message for Nova-LeadArchitect:**
      ```json
      {
        "Context_Path": "SessionEnd (Orchestrator) -> FinalizeDatabaseState (LeadArchitect)",
        "Overall_Project_Goal": "Gracefully end current Nova session.",
        "Phase_Goal": "Update NovaPort-MCP `active_context.state_of_the_union` with final session status.",
        "Lead_Mode_Specific_Instructions": [
          ...
          "Delegate to your NovaPortSteward the task of updating `active_context`. They must first use `get_active_context`, formulate a new `state_of_the_union` string based on my context below and your own knowledge, then use `update_active_context` to log the change."
        ],
    ...
    - **`new_task` message for Nova-FlowAsk:**
      ```json
      {
        ...
        "Mode_Specific_Instructions": [
          ...
          "  - Key NovaPort-MCP Items Created/Updated This Session (Type and ID/Key): [List provided by Orchestrator]",
        ...
        "Required_Input_Context": {
          ...
          "Orchestrator_Key_NovaPort-MCP_Changes_This_Session": "[Summary or list of IDs/keys]",
          "Orchestrator_Key_Decisions_This_Session": "[Summary or list of IDs]",
          "NovaPort-MCP_State_Of_Union": "[String from Nova-LeadArchitect's update]",
...
4.  **Nova-Orchestrator: Inform User and Final `attempt_completion`**
    - **Actor:** Nova-Orchestrator
    - **Action:**
      - Inform user: "Session ending. Current project status has been updated in NovaPort-MCP, and a summary of this session has been saved to `[.nova/summary/FILENAME.md]`."
