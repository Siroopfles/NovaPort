...
- No existing database or minimal data for the `ACTUAL_WORKSPACE_ID`.
...
1.  **Nova-Orchestrator: Delegate Initial Project & Database Setup**
    - **Action:**
    ...
    - **Task:** "Delegate the complete initial setup of the new project's database, directory structure, essential configurations (`ProjectConfig`, `NovaSystemConfig`), and initial `ProductContext` to Nova-LeadArchitect."
    - **`new_task` message for Nova-LeadArchitect:**
      ```json
      {
        ...
        "Phase_Goal": "Initialize database, establish directory structure (if needed), define ProjectConfig & NovaSystemConfig, and draft initial ProductContext for Project [UserProvided_ProjectName].",
        ...
        "Expected_Deliverables_In_Attempt_Completion_From_Lead": [
          "Confirmation of database bootstrap completion (if performed).",
          "Confirmation that `ProjectConfig:ActiveConfig` (key) and `NovaSystemConfig:ActiveSettings` (key) are logged.",
          "NovaPort-MCP key of the initial `ProductContext` (usually 'product_context').",
          "List of other key NovaPort-MCP items created (Decision IDs, Progress IDs, CustomData keys).",
...
2.  **Nova-Orchestrator: Delegate System Design Phase**
    - **DoR Check:** `ProductContext` (key), `ProjectConfig:ActiveConfig` (key), `NovaSystemConfig:ActiveSettings` (key) exist in NovaPort-MCP. User confirms readiness for design.
    ...
    - **`new_task` message for Nova-LeadArchitect:**
      ```json
      {
        ...
        "Lead_Mode_Specific_Instructions": [
          "Your goal for this phase is to define and document the complete system architecture for project '[UserProvided_ProjectName]'. Create a high-level plan for this phase, log it to NovaPort-MCP, and then use your standard single-step execution loop to delegate atomic tasks to your specialists. You may consult `.nova/workflows/nova-leadarchitect/WF_ARCH_SYSTEM_DESIGN_PHASE_001_v1.md` for a reference process. Ensure all design artifacts are created and logged according to project standards."
        ],
        "Required_Input_Context": {
          "ProjectName": "[UserProvided_ProjectName]",
          "NovaPort-MCP_ProductContext_Key": "product_context",
          "NovaPort-MCP_ProjectConfig_Key": "ProjectConfig:ActiveConfig",
          "NovaPort-MCP_NovaSystemConfig_Key": "NovaSystemConfig:ActiveSettings"
        },
        "Expected_Deliverables_In_Attempt_Completion_From_Lead": [
          "Summary of the defined architecture.",
          "List of key NovaPort-MCP items created: `SystemArchitecture` keys, `APIEndpoints` keys, `DBMigrations` keys, core architectural `Decision` (integer `id`s).",
          "Confirmation that `active_context.state_of_the_union` is updated."
        ]
      }
      ```
      ...
      - **Path B (NO - Failure):**
        ...
        - **`new_task` message for Nova-LeadArchitect:**
          ```json
          {
            ...
            "Expected_Deliverables_In_Attempt_Completion_From_Lead": [
              "Confirmation that all failed DoR criteria are now met."
            ]
          }
          ```
...
4.  **Nova-Orchestrator: Delegate Development Phase**
    ...
    - **`new_task` message for Nova-LeadDeveloper:**
      ```json
      {
        ...
        "Lead_Mode_Specific_Instructions": [
          "Your goal for this phase is to implement the core features for project '[UserProvided_ProjectName]'. Create a high-level plan for this phase, log it to NovaPort-MCP, and then use your standard single-step execution loop to delegate atomic tasks to your specialists. You may consult `.nova/workflows/nova-leaddeveloper/WF_DEV_FEATURE_IMPLEMENTATION_LIFECYCLE_001_v1.md` for a reference process. Ensure all code is implemented according to specifications, is fully tested, and meets quality standards."
        ],
        "Required_Input_Context": {
          "ProjectName": "[UserProvided_ProjectName]",
          "NovaPort-MCP_SystemArchitecture_References": "[Provide keys to relevant SystemArchitecture documents]",
          "NovaPort-MCP_APIEndpoints_References": "[Provide keys or tag for APIEndpoints]",
          "NovaPort-MCP_DBMigrations_References": "[Provide keys for DBMigrations]",
          "NovaPort-MCP_ProjectConfig_Key": "ProjectConfig:ActiveConfig"
        },
        "Expected_Deliverables_In_Attempt_Completion_From_Lead": [
...
**Phase 3.5: Definition of Ready (DoR) Check for QA Phase** 5. **Nova-Orchestrator: Verify DoR for QA**
_ **Trigger:** Activated before delegating the QA phase.
_ **Step 1 & 2: Get & Verify DoR Criteria**
_ **Action:** Retrieve `CustomData ProjectStandards:DefaultDoR`. Check criteria like "Development Complete (All planned features implemented and unit/integration tested)", "Code merged to test branch", "Test Environment Ready".
_ Verify by checking `Progress` from development phase is "DONE", and `ProjectConfig` specifies a ready test environment.
_ **Step 3: Conditional Gateway**
_ **Path A (YES - Success):** Log success, proceed to Phase 4. \* **Path B (NO - Failure):** Pause workflow. Delegate remediation task to `Nova-LeadDeveloper` (e.g., "Complete unfinished components", "Fix failing integration tests") or `Nova-LeadArchitect` (e.g., "Ensure test environment in `ProjectConfig` is correctly defined and available"). Await completion, then loop back to start of Phase 3.5.
...
6.  **Nova-Orchestrator: Delegate QA Phase**
    ...
    - **`new_task` message for Nova-LeadQA:**
      ```json
      {
        ...
        "Lead_Mode_Specific_Instructions": [
          "Your goal for this phase is to ensure the quality of project '[UserProvided_ProjectName]'. Create a high-level test plan, log it to NovaPort-MCP, and then use your standard single-step execution loop to delegate atomic testing and bug investigation tasks to your specialists. You may consult `.nova/workflows/nova-leadqa/WF_QA_FULL_REGRESSION_TEST_CYCLE_001_v1.md` or other relevant QA workflows for reference processes. Ensure all new features are tested and all discovered bugs are managed according to project standards."
        ],
        "Required_Input_Context": {
          "ProjectName": "[UserProvided_ProjectName]",
          "NovaPort-MCP_FeatureScope_References": "[Keys or tag]",
          "NovaPort-MCP_AcceptanceCriteria_References": "[Keys or tag]",
          "NovaPort-MCP_APIEndpoints_References": "[Keys or tag for testing]",
...
- Reads all types of NovaPort-MCP items to inform DoR checks and delegation.
