# Workflow: NovaPort-MCP Schema Migration (WF_ARCH_NOVAPORT_MCP_SCHEMA_MIGRATION_001_v1)

**Goal:** To migrate all NovaPort-MCP `CustomData` items within a specific category from an old schema version to a new one, ensuring data integrity and consistency.
...
- **Goal:** Retrieve all items from the target category, apply the transformation logic, and update them in NovaPort-MCP.
- **Suggested Specialist Sequence & Briefing Guidance:**
  ...
  2.  **Delegate to `Nova-SpecializedNovaPortSteward`:**
      ...
      - **Briefing Details:** Instruct the specialist to:
        ...
        - **Batch Update Items:** Use the `batch_log_items` NovaPort-MCP tool to efficiently write all the updated items back to NovaPort-MCP in a single call. This will overwrite the old items.
...
- **Suggested Lead Action:**
  ...
  2.  Delegate a task to `Nova-SpecializedNovaPortSteward` to update the status of the original `ConPortSchema:[key]` proposal item to 'Implemented'.
