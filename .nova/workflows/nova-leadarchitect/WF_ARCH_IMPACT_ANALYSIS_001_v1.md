...
**Goal:** To assess and document the potential impact of a proposed significant change on the project, including effects on code, NovaPort-MCP items, documentation, and project timelines/risks.

**Primary Actor:** Nova-LeadArchitect
**Primary Specialist Actors (delegated to by Nova-LeadArchitect):** Nova-SpecializedSystemDesigner, Nova-SpecializedNovaPortSteward, (potentially Nova-FlowAsk).
...
- **Goal:** Verify that the proposed change is clearly defined in NovaPort-MCP before starting the analysis.
- **Suggested Lead Action:**
  1.  Your first action MUST be to perform a "Definition of Ready" check.
  2.  Retrieve the NovaPort-MCP item describing the proposed change (e.g., `Decision:[ID]` or `FeatureScope:[Key]`) using `use_mcp_tool`.
  3.  **Gated Check:**
...
- **Goal:** Identify all potentially affected NovaPort-MCP items and source code areas.
- **Suggested Specialist Sequence & Briefing Guidance:**
  ...
  2.  **Delegate to `Nova-SpecializedNovaPortSteward`:**
      - **Subtask Goal:** "Identify and list all NovaPort-MCP items potentially affected by the proposed change."
...
- **Suggested Lead Action:**
  1.  Consolidate the lists of impacted NovaPort-MCP items and code areas from your specialists.
  2.  **Analyze & Assess:**
  ...
  4.  Log any significant intermediate findings or the final recommendation strategy as a formal `Decision` in NovaPort-MCP.

**Milestone IA.3: Documentation & Reporting**

- **Goal:** Compile all findings into a structured Impact Analysis Report and log it to NovaPort-MCP.
- **Suggested Specialist Sequence & Briefing Guidance:**
  1.  **Delegate to `Nova-SpecializedNovaPortSteward`:**
      - **Subtask Goal:** "Compile and log the final Impact Analysis Report to NovaPort-MCP."
      ...
      - Return the NovaPort-MCP key of the logged `ImpactAnalyses` report.
