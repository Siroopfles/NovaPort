# Workflow: NovaPort-MCP Data Hygiene Review (WF_ARCH_NOVAPORT_MCP_DATA_HYGIENE_REVIEW_001_v1)

**Goal:** To periodically scan NovaPort-MCP for stale or outdated information, log these items as archival candidates, and present them to the user for a decision on whether to archive them.
...
- Nova-Orchestrator delegates a "database data cleanup" or "archive old items" task.
- A NovaPort-MCP Health Check (`WF_ARCH_NOVAPORT_MCP_HEALTH_CHECK_001_v1.md`) reveals a high volume of outdated items.

**Reference Milestones for your Single-Step Loop:**

**Milestone DH.1: Scan for Stale Items**

- **Goal:** Identify and log all NovaPort-MCP items that meet the project's staleness criteria.
- **Suggested Specialist Sequence & Briefing Guidance:**
  ...
  2.  **Delegate to `Nova-SpecializedNovaPortSteward`:**
      - **Subtask Goal:** "Scan NovaPort-MCP for items that meet staleness criteria and log them as `ArchivalCandidates`."
...
- **Suggested Lead Action:**
  1.  Receive the list of `ArchivalCandidates` from the `NovaPortSteward`.
...
- **Goal:** Mark all user-approved items as archived in NovaPort-MCP.
- **Suggested Specialist Sequence & Briefing Guidance:**
  1.  **Delegate to `Nova-SpecializedNovaPortSteward`:**
      - **Subtask Goal:** "For each user-approved `ArchivalCandidates` key, update the original NovaPort-MCP item to mark it as archived."
      - **Briefing Details:** Instruct the specialist to:
...
- Updates to various existing NovaPort-MCP items (`Decisions`, `SystemArchitecture`, etc.)
